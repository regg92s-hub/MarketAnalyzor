...
def _screener_row(r, kind):
    ...
    def badge(val, ok, label, suffix="%"):
        if val is None:
            return f'<span class="muted" style="font-size:10.5px">{label}: ukjent</span>'
    ...
    insider_html = ('<span style="color:' + PALETTE["up"] + ';font-weight:700">👤 Kjøp</span>' if ins is True
                          ...
                          else '<span class="muted" style="font-size:10px">n/a</span>'))
    name = html.escape(r.get("display_name", r["ticker"]))
    ...
    if kind == "growth":
        ...
        qual = r.get("growth_qualified")
        badges = (badge(r.get("rev_yoy"), (r.get("rev_yoy") or -999) > 50, "YoY")
        ...
        badges = (badge(r.get("eps_yoy"), (r.get("eps_yoy") or -999) > 50, "EPS YoY")
                 + " " + badge(r.get("eps_qoq"), (r.get("eps_qoq") or -999) > 40, "EPS QoQ")
                 + " " + badge(r.get("margin"), (r.get("margin") or -999) > 10, "Margin")
                 + " " + (f'<span style="color:{PALETTE["up"] if (r.get("de") or 99) < 1.2 else PALETTE["down"]};'
                          f'font-size:10.5px;font-weight:600">D/E: {r["de"]:.2f}</span>' if r.get("de") is not None
...
def render_screener(data) -> str:
    ...
    out.append('<p class="sub" style="margin-top:8px">Kilder: yfinance (fundamentaldata), '
               ...
               'analysor/stock_universe.py. <strong>Ikke finansrådgivning.</strong></p>')
    out.append(layout.foot())
    return "".join(out)
